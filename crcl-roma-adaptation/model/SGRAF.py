...
class EncoderTextBert(nn.Module):
    ...
    def forward(self, captions, lengths):
        ...
        embeddings = self.linear(self.bert(input_ids=captions, attention_mask=attention_mask).last_hidden_state)
        return embeddings if self.no_txtnorm else l2norm(embeddings, dim=-1)
...
class SGRAF(object):
    ...
    def __init__(self, opt):
        ...
        if getattr(opt, 'text_enc_type', 'bigru') == 'bert':
            self.txt_enc = EncoderTextBert(opt.embed_size, opt.bert_path, opt.no_txtnorm)
        else:
        ...
        self.sim_enc = EncoderSimilarity(opt.embed_size, opt.sim_dim,
                                         opt.module_name, opt.sgr_step, opt.num_regions)
