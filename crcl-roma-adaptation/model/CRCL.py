...
class CRCL(object):
    ...
    def train_self(self,  images, img_lengths,  targets, cap_lengths,  text_ids, epoch=None, schedule=0):
        ...
        k = images.size(0)
        device = next(self.base_model.img_enc.parameters()).device
        y_hard = torch.eye(k, device=device)
        ...
        eps = 1e-10
        y_last = torch.as_tensor(self.move_gt[list(text_ids)], dtype=torch.float32, device=device)
