...
class RoMaPrecompDataset(data.Dataset):
    def __init__(self, data_root, data_split, vocab, opt):
        bundle = load_roma_bundle(data_root, opt.data_name, data_split)
        self.vocab = vocab
        self.data_split = data_split
        self.module = opt.module_name
        self.text_enc_type = getattr(opt, "text_enc_type", "bigru")
        ...
        self.real_correspondence = np.ones(self.length, dtype=np.float32)
    ...
    def _bigru_caption(self, caption):
        tokens = ["<start>"] + self._tokens(caption) + ["<end>"]
        return torch.tensor([self.vocab(token) for token in tokens], dtype=torch.long)

    def _bert_caption(self, caption):
        tokens = []
        for token in self.vocab.basic_tokenizer.tokenize(caption):
            tokens.extend(self.vocab.wordpiece_tokenizer.tokenize(token))
        ids = self.vocab.convert_tokens_to_ids(["[CLS]"] + tokens + ["[SEP]"])
        return torch.tensor(ids, dtype=torch.long)

    def __getitem__(self, index):
        image = np.array(self.images[self.scene_indices[index]], copy=True)
        if self.data_split == "train":
            image[np.random.rand(image.shape[0]) < 0.20] = 1e-10
        caption = self.captions[index]
        target = self._bert_caption(caption) if self.text_enc_type == "bert" else self._bigru_caption(caption)
        return torch.tensor(image, dtype=torch.float32), target, index
...
def get_precomp_loader(data_path, data_split, vocab, opt, batch_size=100,
    ...
    is_roma = canonical_dataset_name(opt.data_name) in {"scenedepict", "scanrefer", "nr3d", "3dllm"}
    if is_roma:
        dset = RoMaPrecompDataset(getattr(opt, "data_root", "") or data_path, data_split, vocab, opt)
        if data_split == "train":
...
def get_loaders(data_name, vocab, batch_size, workers, opt,logger):
    # get the data path
    dpath = getattr(opt, "data_root", "") or os.path.join(opt.data_path, data_name)
    if canonical_dataset_name(data_name) not in {"scenedepict", "scanrefer", "nr3d", "3dllm"}:
